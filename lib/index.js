...
const getConfigPath = configPaths => {
  ...
  configPaths.some(configPath => {
    const resolvedConfigPath = (0, _path.resolve)(process.cwd(), configPath);
...
var _default = (0, _helperPluginUtils.declare)(api => {
  ...
  return {
    ...
    pre(state) {
      ...
      const configPaths = config ? [config].concat(DEFAULT_CONFIG_NAMES) : DEFAULT_CONFIG_NAMES;
    ...
    visitor: {
      CallExpression(path, state) {
        ...
        if (configPath && filename === (0, _path.resolve)(configPath)) {
          return;
